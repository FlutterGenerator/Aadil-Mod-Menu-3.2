...
void startService(JNIEnv *env, jobject ctx){
    jclass native_context = env->GetObjectClass(ctx);
    jclass intentClass = env->FindClass(OBFUSCATE("android/content/Intent"));
    jclass actionString = env->FindClass(OBFUSCATE("com/aadil/Launcher"));
    ...
    if (api_level() >= 26) {
        // Android 8.0+: startService() throws IllegalStateException if the
        // app is in the background. startForegroundService() is required
        // instead — but the Service itself MUST call startForeground()
        // within a few seconds of onCreate(), or the system kills the app
        // with "Context.startForegroundService() did not then call
        // Service.startForeground()".
        jmethodID startForegroundServiceMethodId = env->GetMethodID(native_context, OBFUSCATE("startForegroundService"), OBFUSCATE("(Landroid/content/Intent;)Landroid/content/ComponentName;"));
        env->CallObjectMethod(ctx, startForegroundServiceMethodId, intent);
    } else {
...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    setText(env, title, OBFUSCATE("<b>Modded by Gamer Aadil</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "Auto Offset Finder for Subway Surf Sample Menu V1 | By Gamer Aadil</p>"
                                     "</marquee></b>"));
    ...
    //Toast Example
    Toast(env,ctx,OBFUSCATE("Modded by Gamer Aadil"),ToastLength::LENGTH_LONG);

    initValid = true;
}

// ---------------------------------------------------------------------
// Launcher.java's "CONNECT" button loads Aadil() in a WebView, then
// waits for the page title to contain Aadil2(). Instead of hitting a
// real server, we load a local data: page whose <title> already
// matches Aadil2() — so the check passes instantly, offline, every time.
// ---------------------------------------------------------------------
jstring Aadil(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(OBFUSCATE("data:text/html,<title>OK</title>"));
}

jstring Aadil2(JNIEnv *env, jobject thiz) {
